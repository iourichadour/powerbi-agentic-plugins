...
tools: Read, Edit, Bash, Grep, Glob, WebFetch, WebSearch, Task, mcp__atlassian-rovo-mcp, mcp__atlassian-mcp-server
model: sonnet
---
...
| Git / PowerShell or Bash | Branch validation and creation | Always |
...
5. **On an explicit PBIP commit/PR summary request** — match the user's
   ...
   3. The prompt will invoke
      `.\github\scripts\pbip-pr-summary\commit_diff_summary.ps1` (or the
      `.sh` equivalent on Bash-based environments) to generate a
      deterministic PBIP diff, then synthesize a Jira-prefixed commit
...
## Cross-platform execution convention
Helper scripts are provided as native pairs: a `.ps1` PowerShell script for
Windows/GitHub Copilot CLI environments, and an equivalent `.sh` Bash script
for Claude Code and other Bash-based environments. Use whichever matches the
current environment:
