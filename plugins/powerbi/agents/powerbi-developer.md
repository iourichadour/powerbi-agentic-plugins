...
1. **Locate the spec** вЂ” Verify the spec file exists at the given path. If not, stop and inform the user.
2. **Review the spec** вЂ” Read the full spec document to understand requirements, design, data sources, and components.
3. **Check for a task plan** вЂ” Look for a Tasks section in the spec.
...
4. **Execute tasks** вЂ” Implement each task using the appropriate skills (semantic model, report, fabric-cli). Delegate any `setup`/`sync`/developer-certification/`generate`/`run` measure-test task to the `pql-tester` agent rather than authoring or running DAX Query View tests yourself.
   - After completing each task, **validate its output against that task's stated acceptance criteria** (or, if the task has no explicit acceptance criteria, the relevant requirement/scenario in the spec) before marking it done.
     ...
     - If the validation fails, do **not** mark the task done вЂ” report which criterion failed and why, and leave it unchecked.
...
5. **Execution summary** вЂ” After implementation, produce a summary of work done in `specs/[SpecName].ExecutionSummary.md`.
...
- `powerbi-report-planning`: For "build me a dashboard"-style requests вЂ” guided requirements в†’ spec в†’ approval в†’ build workflow.
...
- `dax-unit-testing` / `dax-test-framework`: For DAX Query View measure test coverage. Do not author or run these tests yourself вЂ” delegate to the `pql-tester` agent (`setup`/`scan`/`sync`/`generate`/`run`/`report`/`diagnose` modes) whenever a spec's task plan calls for measure certification or test execution.