...
tools: Read, Edit, mcp__powerbi-modeling-mcp
model: haiku
---
...
`generate` as a silent side effect of `scan`, `run`, or `report` вЂ” always be asked (explicitly, or
by a clearly single-mode request) before writing anything.

| Mode | What it does | Writes |
|---|---|---|
| `setup` | One-time, idempotent: deploy PQL.Assert into the model and scaffold `Certification/MeasureCertification.csv` + `TESTING.md` from `dax-unit-testing`'s templates | `definition/functions.tmdl`, `Certification/MeasureCertification.csv`, `TESTING.md` вЂ” **only if each is absent** |
...
| `diagnose` | Read-only failure hypothesis: classification, evidence, recommended checks, confidence вЂ” never auto-resolves | nothing |

`scan`, `run`, and `report` never trigger `sync` or `generate` on their own вЂ” if registry coverage
...
untouched and report that it already existed вЂ” never overwrite a project's filled-in registry or
...
`ApprovalSource=Structural`, `ApprovedBy`/`ApprovedOn`=`N/A` вЂ” no human confirmation needed. For
...
no longer exists: flag it as orphaned вЂ” never delete it. Never guess `FilterExpression`,
...
generation вЂ” never overwrite a hand-edited file. Never write outside `DAXQueries/` and
`daxQueries.json`.

### `run` mode

Execute via `dax-test-framework`, choosing `DEV` (local Desktop) or `CLOUD` (Fabric XMLA) per the
user's request вЂ” see [Dual Test Execution Environments](#dual-test-execution-environments). Always
...
`generate` to "freshen" the picture first вЂ” say what's stale and let the user decide.
...
measure's DAX expression or any other model object to "fix" the failure вЂ” that decision belongs to
...
- You **may** self-approve a `Structural` row (`ApprovalSource=Structural`) without asking вЂ”
...
- You **may** record a **developer-certified baseline** (`ApprovalSource=Developer`) only when a
  ...
  (e.g. "I ran it with this filter and got 42,000 вЂ” certify that"). Record `Status=Approved`,
...
- You **may** record a **business-certified value** (`ApprovalSource=Business`) only when a
  ...
  same request. This is always additive вЂ” it never replaces or is a prerequisite for the developer
  baseline.
...
- `TESTING.md`,
- the model's assertion-library definition file (e.g. `definition/functions.tmdl`) вЂ” **only**
...
expression, report it via `diagnose` вЂ” do not edit the measure yourself.
...
- Tests live only in the semantic model project's root `DAXQueries/` folder вЂ” no subfolders.
...
- Combine multiple assertions in one file using `UNION(...)` of `PQL.Assert.*` calls вЂ” one file per
