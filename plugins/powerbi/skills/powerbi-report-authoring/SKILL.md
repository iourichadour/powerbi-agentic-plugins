...
## Task: Create new report from template

The skill ships a ready-made PBIR template in `assets/templateReport/`. Use it as the starting point instead of building from scratch.

1. **Copy template files** — Copy `assets/templateReport/report/definition` and `assets/templateReport/report/StaticResources` to the `*.Report` folder of the new report.
...
2. **Bind to semantic model** — Update `definition.pbir` to reference the target semantic model via `byPath` or `byConnection`.

3. **Validate** — Run `powerbi-report-author validate <path-to-.Report-dir>` to confirm PBIR integrity before opening in Desktop.

4. **Open in Desktop** — Use `powerbi-desktop open "<path>.pbip"` and take a screenshot to verify the template renders correctly.

---

## Task: Analyze report against best practices

Run the BPA script against the report definition using the bundled PBI Inspector executable. If no specific rules are mentioned, use the default rule set in `scripts/bpa-rules-report.json`.
...
1. **Run BPA** — Execute the script, passing the path to the report's `definition` folder:
    ...
    > The skill ships with **PBI Inspector** (`win-x64-CLI`) checked in under `scripts/_tools/PBIInspector/win-x64/CLI/`. The script does not download or install the tool automatically.

    > On a periodic cadence, the script checks GitHub for a newer PBI Inspector release and prints a warning if one is available. It never replaces the bundled executable for you.
