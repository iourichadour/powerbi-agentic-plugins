...
### Scripted BPA rules (Tabular Editor 2.0)

When the user wants to enforce static, repeatable rules (naming conventions, required documentation, anti-pattern prevention) rather than an ad-hoc MCP-driven review, run `scripts/bpa.ps1` against the model instead of steps 1-6 above. If no specific rules are mentioned, use the default rule set in `scripts/bpa-rules-semanticmodel.json`.

**CRITICAL:**
- If there is no semantic model in context, prompt the user for its location.
- If the model is stored on the local file system, select a folder that contains a `database.tmdl`, `model.tmdl`, or `model.bim` file.

```powershell
scripts/bpa.ps1 -models [path to the semantic model] -rulesFilePath [path to the BPA rules json file]
```

If the model is running on a local or remote server:
...
- **Cross-reference `references/dax-query-guidelines.md`** for post-creation validation — run DAX queries to verify measures, relationships, and data.
