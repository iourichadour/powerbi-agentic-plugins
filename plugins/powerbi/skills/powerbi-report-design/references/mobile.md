...
Author `mobile.json` files directly (read → `JSON.parse` → modify → `JSON.stringify` → write, per this repo's JSON-editing rule in `powerbi-report-authoring`), then run `powerbi-report-author validate <path-to-.Report-dir>` to confirm schema integrity.
...
- Mobile-optimized views render only in native iOS/Android apps; the `powerbi-desktop` CLI's screenshot capability shows only the landscape layout, so you cannot verify the portrait layout by screenshot
