...
| Hierarchical categories (e.g., Region > Country > City) | `pivotTable` | Rows expand/collapse, subtotals per level |
...
For richer inline visuals (dumbbell charts, bullet charts, progress bars), use SVG extension measures — see [`custom-visuals.md`](custom-visuals.md) for build-vs-buy guidance. The trade-off: higher development and maintenance overhead vs. richer context. Use only when benefits justify added complexity.

## Matrix-Specific Guidance

### Row Hierarchy

Bind categories in order from broadest to most granular. See `powerbi-report-authoring`'s [`table.md`](../../powerbi-report-authoring/references/table.md) reference for exact `pivotTable` `visual.json` structure. Example bindings: `Rows: Customers.Key Account Name`, `Rows: Customers.Account Name`, `Rows: Products.Product Name`, `Values: Orders.Order Lines`, `Values: Orders.Net Orders`.
...
Use column headers for time periods or categorical pivots: add a column hierarchy binding for `Columns: Date.Calendar Quarter (ie Q1)` in the visual.json query block.
...
| Flat table with repeating parent values | Redundant data, hard to scan | Use matrix (`pivotTable`) with hierarchy |
