...
**Preferred: add targets to the semantic model** using the `semantic-model-authoring` skill or Tabular Editor. Model-level measures are reusable across reports and benefit from server-side evaluation.
...
**If no clear target exists, ask the user** to discuss options. Do not leave KPIs bare -- a number without context is not actionable.
...
To build a contextualized KPI card, use the `kpi` visual type with title "Revenue MTD", positioned at x=20, y=100, width=300, height=160. Bind field roles: `Indicator: __Measures.Actuals MTD`, `Goal: __Measures.Sales Target MTD`, `TrendLine: Date.Date`. See `powerbi-report-authoring`'s [`card.md`](../../powerbi-report-authoring/references/card.md) reference for exact `visual.json` structure and CLI metadata lookups.
...
1. **Query values** with the report's active filters using the `power-bi-dax` skill or Tabular Editor
...
- **Category label only (preferred)**: Hide the visual title. The category label sits below the large number and naturally reads as "3.8M Order Lines". This is cleaner and leaves more room for the value to render at a large font size.
...
This repo has no dedicated SVG/Deneb custom-visuals skill installed; see [`custom-visuals.md`](custom-visuals.md) in this skill for build-vs-buy guidance. The general pattern:
...
| Rich formatting with icons | `card` + SVG extension measures | See [`custom-visuals.md`](custom-visuals.md) for build-vs-buy tradeoffs |
